...
public class CharacterTester {
    ...
    public void start() {
        future = NumberIden.testExecutor.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    double[] confidences;
                    synchronized (NumberIden.network) {
                        confidences = NumberIden.network.query(inputs);
                    }
    ...
    public interface TesterListener {
        void onTest(CharacterImage img, char result); //void onTest(CharacterImage img, char result);
    }
}
